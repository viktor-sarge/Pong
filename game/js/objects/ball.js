...
export default class Ball extends Circle {
	...
	/**
	 * Creates particle effects when the ball bounces.
	 * @param {string} [color] - Optional color override for particles
	 */
	createBounceParticles(color = null) {
		const particleColor = color || this.config.BALL.BOUNCE_PARTICLE_COLOR;
		this.particles.addEmitter(
			this.x,
			this.y,
			this.config.BALL.BOUNCE_PARTICLE_SPEED,
			this.config.BALL.BOUNCE_PARTICLE_SPEED_VARIANCE,
			this.config.BALL.BOUNCE_PARTICLE_LIFESPAN,
			particleColor,
			this.config.BALL.BOUNCE_PARTICLE_COUNT
		);
	}

	getRandomAngle() {
		// Pick one of the provided allowed angle ranges
		const randomRangeIndex = Math.floor(
			Math.random() * this.angleRanges.length
		);
